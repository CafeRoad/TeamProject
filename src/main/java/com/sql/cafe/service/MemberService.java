...
public class MemberService {
	...
	// 회원가입 폼에서 받은 정보에 인증키를 추가하여 유저 인서트.
	public void insertNewMemberToUser(MemberVO memberVO) throws Exception {
		...
		sendMail.setText(new StringBuffer().append("<h1>메일인증</h1>")
				...
				.append("<a href='http://172.16.81.140:8080/cafe/userEmailConfirm?authKey=").append(key)
	...
	// 회원가입 폼에서 받은 정보에 인증키를 추가하여 오너 인서트.
	public void insertNewMemberToOwner(MemberVO memberVO) throws Exception {
		...
		sendMail.setText(new StringBuffer().append("<h1>메일인증</h1>")
				.append(memberVO.getId()).append("님 가입을 환영합니다.<br/>")
				.append("<a href='http://localhost:8080/cafe/ownerEmailConfirm?authKey=").append(key)
				.append("' target='_blank'>이메일 인증 확인</a>").toString());
	...
	// 이메일 인증 키 검증
	public MemberVO updateToOwner(String authkey) throws Exception {
		MemberVO memberVO = new MemberVO();
		memberVO = memberMapper.selectMemberByAuthority(authkey);
		...
		if (memberVO != null) {
			try {
				memberMapper.updateToOwner(authkey);
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
