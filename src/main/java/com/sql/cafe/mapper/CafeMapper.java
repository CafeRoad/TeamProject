...
public interface CafeMapper {
	...
	final String SELECT_WAITING_CAFES_BY_OWNER_ID = "select * from (select * from cafe where owner_id = #{owner_id} and approval = 0) a left outer join (select cafe_id, avg(stars) as avg_star from review) b on a.cafe_id = b.cafe_id";
	...
	@Results(value = { @Result(property = "cafe_id", column = "cafe_id"),
			...
			@Result(property = "operating_time", column = "operating_time"),@Result(property = "avg_star", column = "avg_star"),
	...
	final String SELECT_ADDED_CAFES_BY_OWNER_ID = "select * from (select * from cafe where owner_id = #{owner_id} and approval = 1) a left outer join (select cafe_id, avg(stars) as avg_star from review) b on a.cafe_id = b.cafe_id";
	...
	final String SELECT_ALL_FROM_WAITING_CAFE = "select * from (select * from cafe where approval = 0) a left outer join (select cafe_id, avg(stars) as avg_star from review) b on a.cafe_id = b.cafe_id";
	...
	final String SELECT_ALL_FROM_ADDED_CAFE = "select * from (select * from cafe where approval = 1) a left outer join (select cafe_id, avg(stars) as avg_star from review) b on a.cafe_id = b.cafe_id";
	...
	// 즐겨찾기한 카페들의 정보를 검색.
	final String MY_FAVORITE_CAFES = "select * from (seletct * from cafe where cafe_id in (select cafe_id from favorite where user_id = #{user_id})) a left outer join (select cafe_id, avg(stars) as avg_star from review) b on a.cafe_id = b.cafe_id";
	...
	// 지역이나 이름으로 카페 검색.
	final String SEARCH_BY_CAFE_NAME = "select * from (select * from cafe where cafe_name like concat('%',#{search},'%') or gu like concat('%',#{search},'%') or address like concat('%',#{search},'%')) a left outer join (select cafe_id, avg(stars) as avg_star from review) b on a.cafe_id = b.cafe_id";
