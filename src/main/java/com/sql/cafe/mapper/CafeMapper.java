...
public interface CafeMapper {
	...
	final String SELECT_WAITING_CAFES_BY_OWNER_ID = "select * from (select * from cafe where owner_id = #{owner_id} and approval = 0) a left outer join (select cafe_id, format(avg(stars),2) as avg_star from review group by cafe_id) b on a.cafe_id = b.cafe_id limit #{selectPage}, 10";
	...
	final String SELECT_ADDED_CAFES_BY_OWNER_ID = "select * from (select * from cafe where owner_id = #{owner_id} and approval = 1) a left outer join (select cafe_id, format(avg(stars),2) as avg_star from review group by cafe_id) b on a.cafe_id = b.cafe_id limit #{selectPage}, 10";
	...
	final String SELECT_ALL_FROM_WAITING_CAFE = "select * from (select * from cafe where approval = 0) a left outer join (select cafe_id, format(avg(stars),2) as avg_star from review group by cafe_id) b on a.cafe_id = b.cafe_id limit #{selectPage}, 10";
	...
	final String SELECT_ALL_FROM_ADDED_CAFE = "select * from (select * from cafe where approval = 1) a left outer join (select cafe_id, format(avg(stars),2) as avg_star from review group by cafe_id) b on a.cafe_id = b.cafe_id limit #{selectPage}, 10";
	...
	final String SELECT_WAITING_CAFES_BY_CAFE_ID = "select * from (select * from cafe where cafe_id = #{cafe_id} and approval = 0) a left outer join (select cafe_id, format(avg(stars),2) as avg_star from review group by cafe_id) b on a.cafe_id = b.cafe_id";
	...
	final String SELECT_ADDED_CAFES_BY_CAFE_ID = "select * from (select * from cafe where cafe_id = #{cafe_id} and approval = 1) a left outer join (select cafe_id, format(avg(stars),2) as avg_star from review group by cafe_id) b on a.cafe_id = b.cafe_id";
	...
	// 즐겨찾기한 카페들의 정보를 검색.
	final String MY_FAVORITE_CAFES = "select * from (seletct * from cafe where cafe_id in (select cafe_id from favorite where user_id = #{user_id})) a left outer join (select cafe_id, format(avg(stars),2) as avg_star from review group by cafe_id) b on a.cafe_id = b.cafe_id limit #{selectPage}, 10";
	...
	// 지역이나 이름으로 카페 검색.
	final String SEARCH_BY_CAFE_NAME = "select * from (select * from cafe where approval = 1 and cafe_name like concat('%',#{search},'%') or gu like concat('%',#{search},'%') or address like concat('%',#{search},'%')) a left outer join (select cafe_id, format(avg(stars),2) as avg_star from review group by cafe_id) b on a.cafe_id = b.cafe_id limit #{selectPage}, 10";
	...
	//아티클로 검색하여 카페 정보를 겟.
	final String GET_ARTICLE_CAFES = "select * from (select * from cafe where approval = 1) a left outer join (select cafe_id, format(avg(stars),2) as avg_star from review group by cafe_id) b on a.cafe_id = b.cafe_id where a.cafe_id in (select cafe from article where article_name = #{article_name})";
